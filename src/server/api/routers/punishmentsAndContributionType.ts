...
export const punishmentOrContributionTypeRouter = createTRPCRouter({
    create: protectedProcedure
        ...
        .mutation(async ({ ctx, input }) => {
            ...
            const canCreatePunishmentOrContribution = !!(isUserOwner || isUserManager);

            if (!canCreatePunishmentOrContribution) throw new TRPCError({ code: 'FORBIDDEN' });
    ...
    getAllForTeam: protectedProcedure
        ...
        .query(async ({ ctx, input }) => {
            const isUserInTeam = await AuthUtil.isUserTeamMember(ctx.prisma, input.teamId, ctx.auth.userId);
