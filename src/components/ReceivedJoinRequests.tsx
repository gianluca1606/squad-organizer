...
export const ReceivedJoinRequests = () => {
    ...
    const sentJoinRequests = api.joinRequest.getAllForLoggedInUser.useQuery(undefined, {
        refetchOnMount: false,
        refetchOnWindowFocus: false,
    });
        ...
        return (
            <Card className="relative mt-4 block w-full p-6  sm:w-full xl:w-10/12 2xl:w-8/12">
                <CardHeader>
                    ...
                    <CardDescription>Here you can see all received join requests for the selected team</CardDescription>
                </CardHeader>
                <CardContent>
                    <ul>
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="inline-flex items-center text-base font-semibold text-gray-900 dark:text-white">
                                        <Button
                                            variant={'ghost'}
                                            className="p-2.5"
                                            onClick={() => accpetJoinRequestFunction(joinRequest.joinRequestId)}>
                                            <Check className="h-5 w-5" />
                                        </Button>
                                        <Button
                                            variant={'ghost'}
                                            className="p-2.5"
                                            onClick={() => declineJoinRequestFunction(joinRequest.joinRequestId)}>
                                            <X className="h-5 w-5" />
                                        </Button>
                                    </div>
                                </div>
